...
const cli = require('../packages/cli/dist/index.js');
const testCases = require('./spec');
const { resolveShortPath } = require('./utils');
...
describe('dependency health command line', () => {
    ...
    run(`should ${testCase.name}`, () =>
      testCli(makeArgv(testCase.module, options)).then(
        ({ log, error, exitCode }) => {
          ...
          actual.missingDetails.should.eql(
  ...
  it('should load config from cli argument', () =>
    testCli(
      ...
      log.should.containEql('Dependency Health Report');
      log.should.containEql('Issues: 0');
      error.should.be.empty();
      exitCode.should.equal(0);
    }));
  ...
  it('should output unused dependencies', () =>
    testCli(makeArgv('bad', {})).then(({ log, error, exitCode }) => {
      log.should.containEql('Unused:');
      log.should.containEql('optimist');
  ...
  it('should output unused devDependencies', () =>
    testCli(makeArgv('dev', {})).then(({ log, error, exitCode }) => {
      log.should.containEql('Dev Unused:');
      log.should.containEql('unused-dev-dep');
  ...
  it('should output missing dependencies', () =>
    testCli(makeArgv('missing', {})).then(({ log, error, exitCode }) => {
      log.should.containEql('Missing Dependencies');
      log.should.containEql('missing-dep');
  ...
  it('should recognize JSX file even only pass jsx parser and require detector', () =>
    testCli(
      ...
      logs.join(' ').should.containEql('Issues: 0');
  ...
  it('should find dependencies with special parser', () =>
    testCli(
      ...
      log.should.containEql('Dev Unused:');
      log.should.containEql('eslint-config-unused');
  ...
  it('should find dependencies with special parser', () =>
    testCli(
      ...
      log.should.containEql('Usage Analysis');
      log.should.containEql('Unused: 0 packages');
