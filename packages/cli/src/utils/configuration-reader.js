...
import ConfigurationParsingException from './exceptions/configuration-parsing-exception.js';
...
export function getCliArgs(args, version) {
  return yargs(args)
    .usage('Usage: $0 scan [DIRECTORY]')
    .boolean(['ignore-bin-package', 'skip-missing', 'ci', 'fix'])
    ...
    .describe(
      'only',
      'Comma separated analyzer list: usage,missing,upgrade,abandonment,security',
    )
    ...
    .describe('quiet', 'Suppress output when no issues are found')
    .describe('detectors', 'Comma separated detector list')
    .describe('specials', 'Comma separated special parser list')
    .describe('ci', 'Exit non-zero if issues are found')
    .describe('fix', 'Attempt to auto-fix issues where possible')
